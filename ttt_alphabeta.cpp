...
pair<int, pair<int, int>> minimax_optimization(int currentPlayer, int depth, int alpha, int beta)
{
	...
	for (int i = 0; i < legal_moves.size(); i++) {
		...
		if ( currentPlayer == NOUGHT ) {
			int score = minimax_optimization( CROSS, depth + 1, alpha, beta).first;

			if (best_score < score) {
				best_score = score - depth; //less the depth more the score.. we want max score
				best_move = current_move;
				
				alpha = max(alpha, best_score);
				if (beta <= alpha) { 
					board[current_move.first][current_move.second] = EMPTY;
					break; 
				}
			}

		} else {
			int score = minimax_optimization( NOUGHT, depth + 1, alpha, beta).first;

			if (best_score > score) {
				best_score = score + depth; //less the depth less the score.. we want min score
				best_move = current_move;
...
int main()
{
	...
	while ( !(isFull()) && getState(currentPlayer) == DRAW ) {   //playing state
		...
		if( getState(currentPlayer) == DRAW && !(isFull()) ) {   //if AI player can play
			...
			pair<int, pair<int, int>> ai_move = minimax_optimization( NOUGHT, 0, LOSS, WIN);
